...
public class SelectorProductoCompraController {
    ...
    private void mostrarDetalle(Producto p) {
        ...
        if (lblPrecioFijo != null) {
            if (precioOficialActivo > 0) {
                ...
                lblPrecioFijo.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-text-fill: #2E7D32; -fx-background-color: #F1F8E9; -fx-background-radius: 6; -fx-padding: 6 10 6 10;");
            } else {
                lblPrecioFijo.setText("Sin precio");
                lblPrecioFijo.setStyle("-fx-font-size: 13px; -fx-text-fill: #9E9E9E; -fx-background-color: #F5F5F5; -fx-background-radius: 6; -fx-padding: 6 10 6 10;");
            }
        }
    ...
    private void ocultarAdvertencia() {
        if (lblAdvertencia != null) { lblAdvertencia.setVisible(false); lblAdvertencia.setManaged(false); }
        if (btnAgregarCompra != null) {
            btnAgregarCompra.setDisable(false);
            btnAgregarCompra.setStyle("-fx-background-color: #388E3C; -fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 13px; -fx-background-radius: 6; -fx-cursor: hand; -fx-padding: 10 0 10 0;");
        }
    }
    ...
    public void onSeleccionarProducto(ActionEvent event) {
        ...
        if (precioOficialActivo <= 0) {
            JOptionPane.showMessageDialog(null,
                    "Este producto no tiene precio registrado.\nRegistra el precio en Inventario antes de comprarlo.",
                    "Sin precio", JOptionPane.WARNING_MESSAGE);
            return;
        }
        try {
            int cant = Integer.parseInt(txtCantidad.getText().trim());
            if (cant <= 0) { JOptionPane.showMessageDialog(null, "La cantidad debe ser mayor a 0."); return; }

            if (compraController != null) {
                compraController.recibirProductoDelCatalogo(
    ...
    private void volverACompra() {
        if (contentArea == null) return;
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/com/example/farmaventa/Compra.fxml"));
            Node compraVista = loader.load();
            CompraController nuevoCtrl = loader.getController();
            if (compraController != null) compraController.restaurarEstadoEn(nuevoCtrl);
            contentArea.getChildren().setAll(compraVista);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Error al volver a Compra: " + e.getMessage());
        }
    }
}